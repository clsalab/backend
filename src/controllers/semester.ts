...
import { deleteSemester, getSemester, getSemesters, inserSemester, updateSemester } from "../services/semester.service";
...
const getItems = async (req: RequestExt, res: Response) => {
    try {
        ...
        handleHttp(res, "ERROR_GET_SEMESTER");
    }
};
...
const postItem = async ({ body }: RequestExt, res: Response) => {
    try {
        ...
        const responseItem = await inserSemester(body);
        res.send(responseItem);
    } catch (e: any) {
        if (e.name === 'ValidationError') {
            handleHttp(res, 'VALIDATION_ERROR', e.message);
        } else {
            handleHttp(res, 'ERROR_POST_SEMESTER', e);
        }
    }
}
...
const updateItem = async (req: RequestExt, res: Response) => {
    try {
        ...
        const response = await updateSemester(id, body);
        ...
        handleHttp(res, 'ERROR_UPDATE_SEMESTER', e);
    }
};
...
const deleteItem = async (req: RequestExt, res: Response) => {
    try {
        ...
        const data = response ? response : "NOT_FOUND";
        res.send({ deleteSemester: data, updatingUser } ); // Corregir "updateUser" a "updatingUser"
    } catch (e) {
        handleHttp(res, 'ERROR_DELETE_SEMESTER');
    }
}
